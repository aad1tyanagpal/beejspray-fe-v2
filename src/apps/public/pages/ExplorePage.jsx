...
// ─── Active Filter Pills ──────────────────────────────────────
function FilterPills({ filters, setFilters, type }) {
  const pills = []
  ...
  return (
    <div className="flex flex-wrap gap-2 mb-3">
      ...
      <button onClick={() => setFilters(p => ({ ...p, brands: [], crops: [], subcats: [], priceRange: null, rating: null, inStockOnly: false }))}
...
// ─── Circles Row ──────────────────────────────────────────────
function CircleItem({ label, img, active, onClick }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-2 group flex-shrink-0">
      <div className="overflow-hidden transition-all duration-200"
        ...
        onMouseEnter={e => { if (!active) { e.currentTarget.style.borderColor = C.primary; e.currentTarget.style.boxShadow = `0 0 0 3px ${C.primary}22` } }}
        onMouseLeave={e => { if (!active) { e.currentTarget.style.borderColor = '#E5E7EB'; e.currentTarget.style.boxShadow = 'none' } }}>
...
export default function ExplorePage() {
  const { type, slug } = useParams()   // type = 'category' | 'crop' | 'brand'
  const navigate = useNavigate()

  // ── Derive page meta from type + slug ──
  const pageLabel = useMemo(() => {
    if (type === 'category') return ALL_CATEGORIES[slug]?.label ?? slug
    if (type === 'crop')     return ALL_CROPS.find(c => c.slug === slug)?.label ?? slug
    if (type === 'brand')    return ALL_BRANDS.find(b => b.slug === slug)?.label ?? slug
    return slug
  }, [type, slug])
  ...
  const circles = useMemo(() => {
    if (type === 'category') {
      const cat = ALL_CATEGORIES[slug]
      if (!cat) return []
      return [{ label: 'All', img: cat.img, slug: 'all' }, ...cat.subcategories.map(s => ({ label: s.label, img: s.img, slug: s.label }))]
    }
    if (type === 'crop')  return ALL_CROPS.map(c => ({ label: c.label, img: c.img, slug: c.slug }))
    if (type === 'brand') return ALL_BRANDS.map(b => ({ label: b.label, img: b.img, slug: b.slug }))
    return []
  }, [type, slug])

  // ── State ──
  const [filters, setFilters] = useState({ brands: [], crops: [], subcats: [], priceRange: null, rating: null, inStockOnly: false })
  const [sort,       setSort]      = useState('popular')
  // const [viewType,   setViewType]  = useState('card')

  // ── Reset filters + sort every time the page context changes ──
  useEffect(() => {
  ...
  const activeCircleSlug = type === 'category'
    ? (filters.subcats.length === 1 ? filters.subcats[0] : 'all')
    : slug

  const handleCircleClick = (circle) => {
    if (type === 'category') {
      // Filter within page
      if (circle.slug === 'all') setFilters(p => ({ ...p, subcats: [] }))
      else setFilters(p => ({ ...p, subcats: [circle.label] }))
    } else {
      // Navigate to new explore page
      navigate(`/explore/${type}/${circle.slug}`)
    }
  }
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.subcats.length)    list = list.filter((_, i) => i % filters.subcats.length === 0) // mock subcat filter
    if (filters.crops.length)      list = list.filter((_, i) => i % 2 === 0) // mock crop filter
    ...
    <div className="min-h-screen" style={{ backgroundColor: C.pageBg }}>
      ...
      <div className="max-w-[1400px] mx-auto px-4 pb-8 flex gap-6 items-start">
        ...
        <aside className="hidden lg:block flex-shrink-0 w-60 bg-white rounded-xl border overflow-y-auto"
          ...
          <FilterSidebar key={`${type}-${slug}`} type={type} slug={slug} filters={filters} setFilters={setFilters} />
        </aside>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="overflow-x-auto scrollbar-hide mb-5">
            <div className="flex gap-5 py-1 justify-start lg:justify-center">
              {circles.map(circle => (
                <CircleItem key={circle.slug} label={circle.label} img={circle.img}
                  active={activeCircleSlug === circle.slug || (circle.slug === 'all' && activeCircleSlug === 'all')}
          ...
          <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
            <p className="text-sm text-gray-500">
              ...
              {filters.subcats.length === 1 && <span> in <span className="font-semibold text-gray-800">{filters.subcats[0]}</span></span>}
            </p>
          ...
          {/* ── Active Filter Pills ──────────────────────── */}
          <FilterPills filters={filters} setFilters={setFilters} type={type} />
          ...
          {filtered.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {filtered.map(p => (
                <ProductListCard key={p.id} product={p} wishlist={wishlist} onToggleWishlist={toggleWishlist} />
              ))}
            </div>
          )}
        </div>
        ...
        <>
          ...
          <div className="fixed left-0 right-0 z-50 bg-white rounded-t-2xl flex flex-col"
            ...
            <div className="overflow-y-auto flex-1">
             <FilterSidebar key={`${type}-${slug}-mobile`} type={type} slug={slug} filters={filters} setFilters={setFilters} onClose={() => setDrawerOpen(false)} />
            </div>
